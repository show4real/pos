...
import { addPayment, getClients } from "../../services/invoiceService";
import {
  Col,
  Row,
  Nav,
...
export class AddPayment extends Component {
  ...
  getBalance(){
    ...
    const previous_payment = payments.map(payment => payment.amount_paid).reduce((prev, curr) => prev + curr, 0);
  ...
  onSavePayment = async (e) => {
    e.preventDefault();
    await toast.dismiss();
    const { amount_paid, validation } = this.state;
    await this.setState({
      validation: {
        ...validation,
  ...
  savePayment = () => {
    ...
    const { invoice, amount_paid, total_amount, previous_payment } = this.state;
  ...
  render() {
    const { addPayment, invoice, toggle } = this.props;
    const {
      invoice_id,
